...
export class ComponentCommunicator {
  ...
  addWatcher(event, id, watcher, replace = false) {
    ...
    if (Array.isArray(event)) {
      for (const ev of event) {
        this._addWatcherInner(sanitise(ev), id, watcher, replace);
      }
    } else {
      this._addWatcherInner(sanitise(event), id, watcher, replace);
    }
  }
  ...
  removeWatcher(event, id) {
    ...
    if (this._exists(_event, _id) === true) {
      const tmp = {};
