...
import { getDateOrNull } from './date-utils';
...
export default class FauxTarget {
  ...
  get defaultValue() {
    return (typeof this._otherProps.defaultValue !== 'undefined')
      ? this._otherProps.defaultValue
      : this.value;
  }

  get tagName() { return this._getTAG(); }

  get valueAsDate() { return getDateOrNull(this.value); }
  ...
  get valueAsNumber() {
    ...
    if (isNum(this.value) === true) {
      return forceNum(
        this.value,
        (t === 'string' && this.value.includes('.')),
      );
    }

    return parseInt('ZYX', 10);
  }
