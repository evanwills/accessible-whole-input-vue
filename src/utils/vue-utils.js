...
export const getAutoComp = (input) => {
  ...
  return (isNonEmptyStr(input) === true)
    ? input
    : undefined;
};
...
export const getAvatar = (url, gender, isPrimary = false) => {
  if (isNonEmptyStr(url)) {
    return url;
  }

  if (isPrimary === true) {
    return '/-/media/images/pictograms/people/two-parents.svg';
  }

  let file = '';
  const family = {
    Male: 'early-years-male-1',
    Female: 'early-years-female-1',
    Unspecified: 'early-years-neutral-2',
    Other: 'early-years-neutral-2',
  };

  file = (typeof family[gender] === 'string')
    ? family[gender]
    : family.Other;

  return `/-/media/images/pictograms/people/${file}.svg`;
};

export const getStateModule = (isChild) => { // eslint-disable-line arrow-body-style
  return (isChild === true)
    ? 'Child'
    : 'User';
};
...
/**
 * Scroll the child of a container into view if any of its vertical
 * space is either above or below the visible bounds of its container
 *
 * @param {HTMLElement} child A child node within a scrolling container.
 *
 * @returns {void}
 */
export const vScrollChildIntoView = (child) => {
  const parent = child.parentNode;

  const cTop = child.offsetTop;
  const cBottom = (child.offsetTop + child.clientHeight);
  const pTop = parent.scrollTop;
  const pBottom = parent.scrollTop + parent.clientHeight;

  let scrollTo = null;

  if (cBottom > pBottom) {
    scrollTo = cTop - parent.clientHeight + child.clientHeight + 4;
  } else if (cTop < pTop) {
    scrollTo = cTop;
  }
