...
export default class ExternalBlur {
  ...
  _getHandler(context, collapsed) { // eslint-disable-line class-methods-use-this
    ...
    return (event) => {
      if (context._listening === true
        && context._node.contains(event.target) === false
      ) {
        if (context._doConsole === false) {
          context._dispatch();
        } else {
          ...
          // eslint-disable-line no-console
          console.log('parent:', context._node);
          context._dispatch();
  ...
  get autoUnset() { return this._autoUnset; }

  get logging() { return this._doConsole; }
